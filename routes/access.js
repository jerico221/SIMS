...
router.post("/save", (req, res) => {
  try {
    ...
    let cmd = InsertStatement("access", "a", ["name", "status"]);

    let data = [[name, status]];

    let cmd_exist = SelectStatement("select * from access where a_name=?", [
      name,
    ]);
...
router.put("/status", (req, res) => {
  try {
    ...
    let updateStatement = UpdateStatement("access", "a", ["status"], ["id"]);
...
router.put("/edit", (req, res) => {
  try {
    ...
    let updateStatement = UpdateStatement("access", "a", ["name"], ["id"]);

    let checkStatement = SelectStatement(
      "select * from access where a_name=?",
      [namemodal]
    );
